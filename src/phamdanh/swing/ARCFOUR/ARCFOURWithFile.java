...
public class ARCFOURWithFile {
	...
	public void createKey(int size, String folderPath) throws NoSuchAlgorithmException, IOException {
		 ...
		 * 
		 */
		File file = new File(folderPath + "\\SecretKeyARCFOUR-" + size + "bits.txt");
		try {
			if (!file.exists()) {
				file.createNewFile();
			}
	...
	public void encrypt(String filePath, String folderPath, int keySize) throws NoSuchAlgorithmException,
		...
		try {
			...
			// Write file here
			File file = new File(folderPath + "\\EncryptARCFOUR.txt");
			try {
				if (!file.exists()) {
					file.createNewFile();
				}
...
//			BufferedWriter bufferedWriter = new BufferedWriter(fileWriter);
	...
	public void decrypt(String keyPath, String filePath, String folderPath)
		...
		SecretKey key = new SecretKeySpec(byteKey, 0, byteKey.length, "ARCFOUR");
		try {
			Cipher cipher = Cipher.getInstance("ARCFOUR");
			cipher.init(Cipher.DECRYPT_MODE, key);

			// Write file here
			File file = new File(folderPath + "\\DecryptARCFOUR.txt");
