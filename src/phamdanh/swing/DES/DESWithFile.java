...
public class DESWithFile {
	...
	public void createKey(int size, String folderPath) throws NoSuchAlgorithmException, IOException {
		 ...
		 * 
		 */
		File file = new File(folderPath + "\\SecretKeyDES-" + size + "bits.txt");
		try {
			if (!file.exists()) {
				file.createNewFile();
			}
	...
	public void encrypt(String filePath, String folderPath, int keySize) throws NoSuchAlgorithmException,
		...
		try {
			...
			// Write file here
			File file = new File(folderPath + "\\EncryptDES.txt");
			try {
				if (!file.exists()) {
					file.createNewFile();
				}
...
//			BufferedWriter bufferedWriter = new BufferedWriter(fileWriter);
	...
	public void decrypt(String keyPath, String filePath, String folderPath)
		...
		SecretKey key = new SecretKeySpec(byteKey, 0, byteKey.length, "DES");
		try {
			Cipher cipher = Cipher.getInstance("DES");
			cipher.init(Cipher.DECRYPT_MODE, key);

			// Write file here
			File file = new File(folderPath + "\\DecryptDES.txt");
